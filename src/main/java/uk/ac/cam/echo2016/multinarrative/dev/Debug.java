...
public class Debug {
    ...
    /**
     * Create a new instance of the Debug class, mainly loads in configuration data from the config.json file.
     * 
     */
    private Debug() {
        // initialise the configuration an array, each integer representing which systems should be logged at each
        // level.
        ...
        } catch (IOException | ClassCastException | IllegalStateException e) { // config.json doesn't exist, so log
                                                                               // everything
