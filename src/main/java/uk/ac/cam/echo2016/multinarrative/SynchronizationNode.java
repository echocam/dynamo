 ...
 * 
 * <p>
 * ALT: Implements a {@link StoryNode} at an intersection point on the {@code MultiNarrative} graph structure. At this
 * point, multiple {@code Route}s interact and affect each other's gameplay. There should be at least 2 {@code Route}s
 * entering the {@code Node} for the {@code Node} to be meaningful. The most typical use would be to have the same
 * number of entering {@code Route}s also exiting (one for each character), but this isn't required.
 * 
 * @author tr393
 * @author rjm232
 * @version 1.0
